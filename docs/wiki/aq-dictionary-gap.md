...
**Date:** 2026-04-22  
**Source:** `~/.hermes/obsidian/hermetic/raw/AQ Dictionary (copy 1).md` (83 entries, updated Apr 22)  
**Wiki coverage:** 38 of 83 entries have their numerical values documented in wiki pages (mainly in `alphanumeric-qabbala.md` and `index.md`).
...
- High-value numerosophical constants: 83, 137, 189, 333, 360, 666, 777 are only partially documented.
- System labels: 174 (Numogram), 328 (Alphanumeric Qabbala), 360 (Harmonic Syzygies) — these are self-referential and should be explicit.
- Newer additions: 137=autism, 234=Baroqwerty, 369=Soy Simulation appear to be contemporary insertions.

**Recommendation:** Create a dedicated wiki page `aq-dictionary-reference.md` listing all 83 entries with their AQ values, digital roots, zones, and brief definitions. Link from `alphanumeric-qabbala.md` and `index.md`.

---



## Grok Rotor Findings (2026-04-22)

From the Grok conversation mining @xenocosmography and @doomcrypt's X posts, checked against ciphers.news (Synx) and qliphoth.systems/gematria:
...
The Synx cipher uses a non-linear progression (a=14, b=15, c=18... z=1260). Cross-referencing AQ and Synx reveals **dual-cipher resonance** — terms significant in both systems are doubly charged. The full Synx table is screenshot in `wiki/assets/content-ciphers-news-synx.jpg`.

### Rotational / Strobogrammatic Properties

Seven-segment display digits that rotate to valid digits: {0,1,2,5,6,8,9}. With BEGHILOS extension, 2→S and 5→S. Numbers composed only of these digits are **rotationally symmetric** — they form strobogrammatic gates.

Examples:
- 69 → 69 (rotates to itself)
- 121 → 151 (6↔9 swap)
- 666 → 999 (all 6's become 9's)

These rotational pairs are addressable glyphs in the Djynxxogram.

### Sources

- `raw/Grok rotor.md` — Full 55K conversation with Grok
- `raw/assets/content.jpg` → `wiki/assets/content-ciphers-news-synx.jpg`
