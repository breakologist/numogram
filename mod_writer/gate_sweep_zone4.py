...
tmpdir = Path('/tmp/zone4_gate_sweep')
tmpdir.mkdir(exist_ok=True)
report_path = tmpdir / 'gate_sweep_report.md'

aqs = list(_aq_candidates_for_zone(4, count=5, start_k=0))
print("Zone 4 AQ candidates:", aqs)
lines = []
lines.append("| idx | AQ   | Gate  | BPM  | OnsetRate | NoBase | WithBase |")
lines.append("|-----|------|-------|------|-----------|--------|----------|")

for i, aq in enumerate(aqs):
    composer = ModComposer(title=f"Z4_gate_{i}")
    zc = ZoneComposer(composer)
    zc.target_zone(zone=4, aq_seed=str(aq), duplicate_order=True)
    zc.add_section(length=16, channel=0)
    gate = zc._gate_from_aq(str(aq))
    
    mod_path = tmpdir / f'z4_gate_{i:02d}_aq{aq}.mod'
    composer.write_mod(str(mod_path))
    
    wav_path = tmpdir / f'z4_gate_{i:02d}.wav'
