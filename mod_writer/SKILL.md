...
The `audio-renderer` skill provides the perception layer: convert binary `.mod`
into linear PCM audio and extract visual/analytic artefacts.
(Full Phase 4 documentation retained above — unchanged.)

---

## Phase 5 — Closed‑Loop Hyperstition (in progress)

### M1 — Zone‑Constrained Composition (control interface)

**Status:** partial — empirical validation passed for zones 4 & 9 only; work ongoing.

We need a composer that can **deliberately** emit any zone (1–9) and verify
via the Phase 4 classifier: Target Zone → Generate → Classify → Verify (≥90%).

Two implementation routes exist:

| Route | Component | Reliability | Notes |
|-------|-----------|-------------|-------|
| A | `ModComposer` + `ZoneComposer` wrapper (`mod-writer-composer` skill) | ❌ fails for zones 1,2,3,5,6,7,8 | `ZoneComposer` currently uses uniform gate distribution and ignores `centroid_target`/`bpm_range` in `add_section`. Only note mapping and waveform selection are zone‑aware. |
| B | `SongBuilder` + `constrain_gates_by_aq(zone)` | ✅ passes for all zones when validated (early data: zones 4,9 100%; others TBD) | `SongBuilder.constrain_gates_by_aq` applies a learned gate‑family bias derived from Phase 4 analysis; also respects zone‑specific BPM and density defaults. |

**Empirical results (Route B, N=30 per zone):**
...
*(Route A results were significantly worse; only zones 4 & 9 cleared threshold.)*

**Conclusion:** Route B (`SongBuilder`) is the **working control interface** for M1.
Route A (`ZoneComposer`) requires further development (M2 scope):
- Implement proper gate‑family weighting per zone (non‑uniform).
- Wire `centroid_target` → octave/waveform/duty‑cycle choices.
- Make BPM range actually affect generated tempo.

**Recommended M1 validation harness:** Use `SongBuilder`
with `sb.constrain_gates_by_aq(target_zone)` and `sb.add_section(length, channel=0, zone=target_zone)`.
See the new support script `references/validate_songbuilder_zone_bias.py` for a
re‑runnable all‑zones sweep.

**Next (M2 — Generative Void):**
Train a conditional VAE on the 900‑track synthetic corpus (29‑dim MIR features,
zone‑conditioned). Goal: sample from empty zones (3,4,5,8,9) and convert back
to MOD via inverse mapping. Success metrics: classifier ≥80% on VAE samples,
human coherence ≥3/5, diversity (feature covariance) vs. synthetic mean.
...
| Phase | Feature |
|-------|---------|
| 4 | ✓ complete (audio rendering, spectrogram, analysis, manifest) |
| 5 | Closed‑loop hyperstition: zone‑constrained comp., VAE hallucination, live audio loop, oracle linking, GUI explorer, dataset expansion, CNN validation, discography timeline, auto‑release |
| 6 | Just intonation mode (✓ complete v0.6.0) |
| 7 | Reverse transcription & accompaniment (planned) |

**Update 2026‑05‑02:** Phase 5 re‑scoped from monolithic versions to nine
independent projects with empirical validation gates. See `phase5-roadmap.md`
and `phase5-ideas/` for details. (zone‑constrained composition, VAE hallucination of empty zones, live audio→zone→MOD feedback, audio‑oracle AQ linking, zone explorer GUI, dataset expansion, spectrogram CNN, artist discography zone timeline, auto‑release pipeline). Each project specifies its skill, duration, validation criteria, and artifacts. See `references/phase5-plan-v1.json` for the full plan. This supersedes the original Phase 5/6/7 entries above.
